...
function parseWhenToNYLocal(whenText) {
  ...
  if (!m) {
    throw new Error(
      'No pude leer la hora. Usa: "mañana 3pm" o "2026-02-23 15:00"'
    );
  }
...
function nyLocalToDate(localStr) {
  // Convierte "YYYY-MM-DDTHH:mm:00" a Date (sin timezone real)
  // Sirve para guardar "dueAt" como referencia (no perfecta para TZ, pero estable)
  ...
  const userId = String(msg.from.id);
    ...
    if (/^\/?event\b/i.test(text.trim())) {
      try {
        ...
        // ✅ IMPORTANTE:
        // Con Service Account, invitar attendees puede fallar sin domain-wide delegation.
        // Así que SOLO enviamos invite si tú lo necesitas y si no falla.
        // Si falla por attendees, hacemos fallback creando el evento sin attendees.
